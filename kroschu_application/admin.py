...
class UserAdmin(admin.ModelAdmin):
    list_display = ('user_id', 'nom', 'prenom', 'role', 'created_at' ,'password')  # Affiche les champs dans la liste
    list_editable = ('role',)  # Permet d'éditer directement le rôle dans l'interface admin
    list_filter = ('role',)  # Ajoute un filtre par rôle
    search_fields = ('nom', 'prenom', 'role')  # Permet de rechercher par nom, prénom, ou rôle

admin.site.register(User, UserAdmin)

# Admin pour Demande
class DemandeAdmin(admin.ModelAdmin):
    list_display = ('user_id', 'type_materiel', 'statut', 'created_at')  # Affiche les champs dans la liste
    ...
    search_fields = ('type_materiel',)  # Permet de rechercher par type de matériel

admin.site.register(Demande, DemandeAdmin)

# Admin pour Alerte
class AlertemainAdmin(admin.ModelAdmin):
    list_display = ('user_id','type_alertemain', 'statut', 'created_at')  # Affiche les champs dans la liste
    list_editable = ('statut',)  # Permet d'éditer directement le statut dans l'interface admin
    list_filter = ('statut',)  # Ajoute un filtre par statut
    search_fields = ('type_alertemain',)  # Permet de rechercher par type d'alerte

admin.site.register(Alertemain, AlertemainAdmin)

class AlertequalAdmin(admin.ModelAdmin):
    list_display = ('user_id','type_alertequal', 'statut', 'created_at')  # Affiche les champs dans la liste
    list_editable = ('statut',)  # Permet d'éditer directement le statut dans l'interface admin
    list_filter = ('statut',)  # Ajoute un filtre par statut
    search_fields = ('type_alertequal',)  # Permet de rechercher par type d'alerte

admin.site.register(Alertequal, AlertequalAdmin)

class AlertechefAdmin(admin.ModelAdmin):
    list_display = ('user_id','type_alertechef', 'statut', 'created_at')  # Affiche les champs dans la liste
    list_editable = ('statut',)  # Permet d'éditer directement le statut dans l'interface admin
    list_filter = ('statut',)  # Ajoute un filtre par statut
    search_fields = ('type_alertechef',)  # Permet de rechercher par type d'alerte
